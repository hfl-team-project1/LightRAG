...
You are a Knowledge Graph Specialist. Extract ALL entities and relationships from the input text.

---Output Format---
Entities: `entity{tuple_delimiter}name{tuple_delimiter}type{tuple_delimiter}description`
Relations: `relation{tuple_delimiter}source{tuple_delimiter}target{tuple_delimiter}relation_type{tuple_delimiter}description`

Entity types: `{entity_types}`. Use `Other` if none fit.
Use title case for entity names. Use consistent naming throughout.
{relation_type_instruction}
Multiple relation types on one relation: separate with comma `,` (NOT `{tuple_delimiter}`).

---Rules---
1. Output ALL entities first, then ALL relationships. Do not omit any.
2. When a sentence mentions multiple entities in the same role (e.g., "partners X, Y, and Z"), extract a SEPARATE relation for EACH entity.
3. Decompose N-ary relationships into binary pairs.
4. Do not output duplicate relationships.
5. Use third person. Name entities explicitly — no pronouns.
6. Write output in `{language}`. Keep proper nouns in their original language.
7. Output `{completion_delimiter}` as the final line when done.

---Examples---
{examples}
"""

PROMPTS["entity_extraction_user_prompt"] = """Extract ALL entities and relationships from the text below. Output only the extraction — no commentary.
...
PROMPTS["entity_continue_extraction_user_prompt"] = """Review your previous extraction against the input text. Extract any MISSED entities or relationships. Do NOT repeat already-extracted items. Output only additions — no commentary.
